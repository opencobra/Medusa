...
def iterative_gapfill_from_binary_phenotypes(model,universal,phenotype_dict,
    ...
    returned). Currently only supports gapfilling to positive growth conditions.

    Generally, solutions are easier to find and more likely to exist if users
    ensure that transporters for metabolites either 1) exist in the model already
    or 2) are present in the universal model.
    ...
    phenotype_dict : dict
        ...
        bounds, as set in cobra.core.model.medium. Exchange reactions are provided
        with reaction.id, not cobra.core.reaction objects.
    output_ensemble_size : int
        ...
        constructed, the number of members may be lower than output_ensemble_size
        if any duplicate solutions were found across cycles.
    ...
    exchange_prefix : string, "EX_"
        ...
        reactions. "EX_" is standard for modelSEED models. This will be updated
        to be more database-agnostic when cobrapy boundary determination is
        finalized for cobrapy version 1.0.

    Returns
    -------
    solutions : list
        list of lists; each list contains a gapfill solution for a single cycle.
        Number of lists is equal to output_ensemble_size.
    """
    if gapfill_type not in ["integer","continuous"]:
        raise ValueError("only gapfill types of integer and continuous are supported")
    ...
    for condition in phenotype_dict.keys():
        exchanges_in_phenotype_dict = \
                    exchanges_in_phenotype_dict | set(phenotype_dict[condition].keys())
    ...
    for exchange in exchanges_in_phenotype_dict:
        if not exchange in model_rxns:
            ...
            raise ValueError('phenotype_dict contains exchange reactions not found in model. Add any necessary exchange reactions prior to gapfilling.')
    ...
    while len(cycle_order) < output_ensemble_size:
        condition_list = random.sample(list(phenotype_dict.keys()),len(phenotype_dict.keys()))
    ...
    # our strategy is to reduce the cost for the reactions returned by the previous
    # solution to 0, such that they are automatically included in the model for
    # the next condition.
    if gapfill_type is "integer":
        solutions =  _integer_iterative_binary_gapfill(model,phenotype_dict,cycle_order,
                              universal=universal, output_ensemble_size=output_ensemble_size,
                              lower_bound=lower_bound, penalties=penalties,
                              demand_reactions=demand_reactions,
                              exchange_reactions=exchange_reactions,
                              integer_threshold=inclusion_threshold)
    elif gapfill_type is "continuous":
        solutions = _continuous_iterative_binary_gapfill(model,phenotype_dict,cycle_order,
                              universal=universal, output_ensemble_size=output_ensemble_size,
                              lower_bound=lower_bound, penalties=penalties,
                              demand_reactions=demand_reactions,
                              exchange_reactions=exchange_reactions,
    ...
    ensemble =_build_ensemble_from_gapfill_solutions(model,solutions,universal=universal)
    return ensemble
...
def _continuous_iterative_binary_gapfill(model,phenotype_dict,cycle_order,
    ...
    if exchange_reactions:
        raise NotImplementedError("Inclusion of new exchange reactions is not supported for continuous gapfill")
    if demand_reactions:
        raise NotImplementedError("Inclusion of demand reactions is not supported for continuous gapfill")
    ...
    # not share IDs, so make sure your model and universal are in the same namespace.
    ...
    # we will need to check for flux after solving the problem (e.g. these are the
    # reactions we are considering adding to the model)
    get_fluxes = [rxn.id for rxn in gapfiller.reactions]

    # add the reactions from the model to the gapfiller, which are not included
    # in the pFBA formulation, and thus flux is not penalized through them.
    gapfiller.add_reactions([rxn for rxn in model_to_gapfill.reactions])
    ...
    for cycle_num in range(0,output_ensemble_size):
        ...
        with gapfiller:
            ...
            for condition in cycle_order[cycle_num]:
                #gapfiller.medium = phenotype_dict[condition]
                for ex_rxn in phenotype_dict[condition].keys():
                    ...
                    gapfiller.reactions.get_by_id(ex_rxn).upper_bound = \
                        1.0*phenotype_dict[condition][ex_rxn]
                #print(gapfiller.medium)
                # gapfill and get the solution. The 0 index is necessary because gapfill will
                # return a list of lists; we are only taking the first ()and only) list here.
                iteration_solution = gapfiller.optimize()
                print(iteration_solution)
                filtered_solution = {rxn:iteration_solution.x_dict[rxn] for \
                    rxn in get_fluxes if abs(iteration_solution.x_dict[rxn]) > 1E-8}
                print(filtered_solution)
                # validate that the proposed solution restores flux through the objective
                ...
                for ex_rxn in phenotype_dict[condition].keys():
                    ...
                    model_to_gapfill.reactions.get_by_id(ex_rxn).upper_bound = \
                        1.0*phenotype_dict[condition][ex_rxn]
                if not validate(model_to_gapfill,add_rxns,lower_bound):
                ...
                reaction_variables = ((gapfiller.reactions.get_by_id(rxn).forward_variable,
                ...
                print("reactions included after gapfilling on " + condition + ":")
                print(cycle_reactions)
...
def _integer_iterative_binary_gapfill(model,phenotype_dict,cycle_order,
    ...
    for cycle_num in range(0,output_ensemble_size):
        ...
        for condition in cycle_order[cycle_num]:
            ...
            # iterate through indicators, find those corresponding to the gapfilled
            # reactions from any iteration within this cycle, and reset their cost to 0.
            # Doing this for all reactions from any iteration within the cycle is
            # necessary because cobrapy's gapfill function performs update_costs,
            # which will reset costs and iteratively increase them; without this
            # manual override performed here, costs for previous conditions within
            # a cycle would revert to 1 instead of the desired 0
            for reaction_indicator in [indicator for indicator in gapfiller.indicators]:
...
def _build_ensemble_from_gapfill_solutions(model,solutions,universal=None):
    ...
    for solution in solutions:
        ...
        solution_as_rxn_objs = [universal.reactions.get_by_id(rxn).copy() for rxn in solution]
        solution_dict[solution_id] = DictList() + solution
        i += 1

    # scan through other members and remove them if they are identical.
    # as long as we're looping, we'll also find reactions that are in all solutions
    # and get the list of reactions that are in any ensemble.

    # first, convert the solution dictionary to the same structure except with reaction
    # ids rather than reaction objects so that we can perform set operations
    solutions_as_ids = {}
    ...
    for member_id in solutions_as_ids.keys():
        ...
        #if this is the first ensemble member, set intersection will fail because of the empty set
        else:
    ...
    # perform the removal of duplicate solutions on the original solution object
    # which contains reaction objects rather than reaction ids as strings
    for duplicate in duplicate_solutions:
        solution_dict.pop(duplicate,None)

    # Reactions that need features are those that were not in all the gapfill solutions.
    reactions_needing_features = list(all_reactions - in_all)
    reactions_needing_features_objs = [universal.reactions.get_by_id(rxn).copy() for rxn in reactions_needing_features]

    # add reaction objects to the base model for all reactions
    all_reactions_as_objects = [universal.reactions.get_by_id(rxn).copy() for rxn in all_reactions]
